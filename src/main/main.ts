...
import { setWindow, setOutWindow, setMemoryStore } from './utils/window';
...
const createOutWindow = async () => {
  ...
  outWindow.loadURL(resolveHtmlPath('index.html', 'outWindow'));
...
app
  ...
  .then(async () => {
    ...
    setOutWindow(await createOutWindow());
    ...
    globalShortcut.register('Command+Shift+Down', async () => {
      outWindow?.show();
    });
  })
  .catch(console.log);
